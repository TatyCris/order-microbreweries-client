    ...
    return async function (dispatch) {
        try {
            const response = await Promise.all(breweries.map(brewery => {
                ...
                const url = `https://${mapbox_host}/directions/v5/mapbox/driving/${encodeURIComponent(coordinates)}.json?steps=true&geometries=geojson&access_token=${process.env.REACT_APP_MAPBOX_API_KEY}`
                return request(url)
            }))
